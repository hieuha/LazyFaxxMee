...
| file | vai trò |
|------|------|
| `faxxme/app.py` | route FastAPI, presence WebSocket, logic giao fax, canh máy in, device token, webhook inbound |
| `faxxme/db.py` | SQLite (stdlib): `users` (+ băm device-token, webhook secret, session epoch) + `faxes` (BLOB ảnh đã dither, cờ xóa từng phía) |
| `faxxme/auth.py` | mật khẩu pbkdf2 + cookie phiên hmac (kèm epoch theo user) + device token (không phụ thuộc native) |
| `faxxme/printer.py` | dựng biên nhận ESC/POS (loại byte điều khiển), tự cắt, và local bridge in ra `/dev` |
| `faxxme/imaging.py` | ảnh → halftone 1-bit Floyd–Steinberg → raster `GS v 0` (Pillow); chặn bomb theo số pixel |
...
## Webhook inbound (fax từ blog, v.v.)

Một site bên ngoài cũng có thể gửi fax thay cho người không có tài khoản, qua `POST /api/fax/inbound`.
Nó xác thực bằng **webhook secret** của tác giả nhận (token `Bearer fxwh_…`, không phải phiên), gộp
message + ghi nguồn thành một bản fax từ tài khoản hệ thống dành riêng `webhook`, rồi đưa vào cùng
`deliver()` ở trên. Giới hạn tần suất theo tác giả và theo IP site gọi; nội dung được làm sạch trước
khi in. Thiết kế đầy đủ trong [Tích hợp webhook](webhook.md).
